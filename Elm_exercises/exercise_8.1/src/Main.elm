...
attributeValue : Attribute -> Car -> Float
attributeValue attr car =
    case attr of
        CityMPG ->
        ...
        DealerCost ->
            car.dealerCost |> Maybe.withDefault 0 |> toFloat
        ...
        CarWidth ->
            car.carWidth |> Maybe.withDefault 0 |> toFloat

        EngineSize ->
            car.engineSize |> Maybe.withDefault 0
...
update msg model =
    case msg of
        SelectX attr ->
            { model | xAttribute = attr }

        SelectY attr ->
            { model | yAttribute = attr }
    ...
    let
        completeCars =
            List.filter
                (\car ->
                    List.all (\x -> x)
                        [ car.cityMPG /= Nothing
                        , car.retailPrice /= Nothing
                        , car.dealerCost /= Nothing
                        , car.carLen /= Nothing
                        , car.weight /= Nothing
                        , car.carWidth /= Nothing
                        , car.engineSize /= Nothing
                        ]
                )
                myCars
        ...
        ergebnis =
            List.map
                ...
                completeCars
    in
    { xDescription = attributeToString xAttr
    , yDescription = attributeToString yAttr
    , data = ergebnis
    }
