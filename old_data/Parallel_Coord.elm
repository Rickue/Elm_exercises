module Parallel_Coord exposing (main)
...
parallelCoodinatesPlot width height model =
    ...
    svg [ viewBox 0 0 (width + 2 * padding) (height + 2 * padding), TypedSvg.Attributes.width <| TypedSvg.Types.Percent 100, TypedSvg.Attributes.height <| TypedSvg.Types.Percent 100 ]
        [ style []
            [ TypedSvg.Core.text
                (".pc-line { stroke: rgba(0, 0, 0, 0); stroke-width: 1.5; fill: none; }\n"
...
completeEntries : List Car -> List MultiDimPoint
            ...
            (\car ->
                Maybe.map4
                    (\city retail dealer len ->
                        ...
                        , value = [ toFloat city, toFloat retail, toFloat dealer, toFloat len ]
                        }
                    )
                    car.cityMPG
                    car.retailPrice
...
main =
    ...
    Html.div []
        [ Html.p []
            [ Html.text ("Gewählte Autoklasse: " ++ class1Name ++ " | Anzahl der Autos in dieser Klasse: " ++ String.fromInt (List.length filteredCars)) ]
        , parallelCoodinatesPlot w h model
        ]
