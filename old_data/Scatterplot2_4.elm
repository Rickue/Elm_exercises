...
pointStar : ContinuousScale Float -> ContinuousScale Float -> String -> Point -> Svg msg
pointStar scaleX scaleY pointClass xyPoint =
    let
        px =
    ...
    g [ class [ "point", pointClass ], fontSize <| Px 10.0, fontFamily [ "sans-serif" ] ]
        [ case pointClass of
            "above" ->
                text_
                    [ class [ "star-shape" ] -- <--- DIESE ZEILE HINZUFÜGEN
                    , x px
                    , y (py + 6)
                    , textAnchor AnchorMiddle
                    , fontSize <| Px 18.0
...
pointAnimation : ContinuousScale Float -> ContinuousScale Float -> String -> Point -> Svg msg
pointAnimation scaleX scaleY pointClass xyPoint =
    let
        px =
            Scale.convert scaleX xyPoint.x
...
scatterplotV1 model =
    renderPlot
        "plot-v1"
        ".plot-v1 .point.above text.star-shape { fill: rgb(255, 215, 0); stroke: rgb(184, 134, 11); stroke-width: 1px; display: inline; }\n .plot-v1 .point.other circle, .plot-v1 .point.other rect, .plot-v1 .point.other polygon {stroke: rgba(0, 0, 0,0.4); fill: rgb(240, 240, 240); }\n .plot-v1 .point.below circle, .plot-v1 .point.below rect, .plot-v1 .point.below polygon {stroke: rgba(0, 0, 0,0.4); fill: rgb(180, 180, 180); }"
        pointStar
        model
...
scatterplotV2 model =
    renderPlot
        ...
        """
        @keyframes testGlow {
        0% { fill: rgba(255, 247, 0, 0.4); stroke: rgba(255, 247, 0, 1.0); stroke-width: 1px; }
        50% { fill: rgba(5, 161, 130, 1.0); stroke: rgba(5, 161, 130, 1.0); stroke-width: 4px; }
        100% { fill: rgba(255, 247, 0, 0.4); stroke: rgba(255, 247, 0, 1.0); stroke-width: 1px; }
        }
        .plot-v2 .point.above circle { animation: testGlow 1.5s infinite; }
        .plot-v2 .point.other circle {stroke: rgba(0, 0, 0,0.4); fill: rgb(240, 240, 240); }
        .plot-v2 .point.below circle {stroke: rgba(0, 0, 0,0.4); fill: rgb(180, 180, 180); }
        """
        pointAnimation
        model
...
main =
    ...
    Html.div []
        ...
        , Html.h3 [] [ Html.text "Version 1: Test Animation" ]
        , scatterplotV2 filteredCars
        , Html.h3 [] [ Html.text "Version 2: Stern" ]
