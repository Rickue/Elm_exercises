    ...
    svg [ viewBox 0 0 w h, TypedSvg.Attributes.width <| TypedSvg.Types.Percent 100, TypedSvg.Attributes.height <| TypedSvg.Types.Percent 100 ]
        ...
        , g [ transform [ Translate padding (h - padding) ] ]
            ...
            , text_
                ...
                , y (Scale.convert xScaleLocal labelPositions.x)
        ...
        , g [ transform [ Translate padding padding ] ]
            [ yAxis yValues
            , text_
                [ x (Scale.convert yScaleLocal labelPositions.y)
        ...
        ergebnis =
            List.filterMap
                (\car ->
                    Maybe.map3
                        (\name x y ->
                            { pointName = name ++ " (" ++ String.fromInt x ++ "," ++ String.fromInt y ++ ")"
                            , x = toFloat x
                            , y = toFloat y
                            }
                        )
                        (Just car.vehicleName)
                        car.cityMPG
                        car.retailPrice
                )
                my_cars
    in
    XyData "cityMPG" "retailPrice" ergebnis
...
main =
    let
        ...
        numberCars =
            Html.text (String.fromInt (List.length cars))

        numberFilterCars =
            Html.text (String.fromInt (List.length filteredCars.data))
    in
    Html.div []
        [ Html.p []
            [ Html.text "Total Cars: ", numberCars, Html.text " | ", Html.text "Filtered Cars: ", numberFilterCars ]
