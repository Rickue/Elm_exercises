module QQPlot4_1 exposing (main)
    ...
    svg [ class [ "qq-plot" ], viewBox 0 0 w h, TypedSvg.Attributes.width <| TypedSvg.Types.Percent 100, TypedSvg.Attributes.height <| TypedSvg.Types.Percent 100 ]
        ...
        , g [ transform [ Translate padding padding ] ]
            ...
            , text_
                ...
                [ TypedSvg.Core.text ("Empirische Quantile (" ++ className ++ " cityMPG)") ]
            ]
        , g [ transform [ Translate padding padding ] ]
            [ line
                [ class [ "diag" ]
                , x1 (Scale.convert scaleXLocal dMin)
                , y1 (Scale.convert scaleYLocal yMin)
                , x2 (Scale.convert scaleXLocal dMax)
                , y2 (Scale.convert scaleYLocal yMax)
                ]
                []
            ]
        , g [ transform [ Translate padding padding ] ]
...
main =
    let
        ...
        class1Name : String
        class1Name =
            carTypeToString chosenCarType

        class1Values =
            List.map .cityMPG filteredCars1.chosenComplete
    in
    Html.div []
        [ Html.p []
            [ Html.text ("Gewählte Autoklasse: " ++ class1Name ++ " | Anzahl der Autos in dieser Klasse: " ++ String.fromInt (List.length class1Values)) ]
        , Html.h3 [] [ Html.text ("Normal-QQ-Plot für den Stadtverbrauch (cityMPG) der Klasse " ++ class1Name) ]
        , normalQQPlotFromValues class1Name class1Values
        ]
