...
import Statistics
import TypedSvg exposing (circle, g, polygon, rect, style, svg, text_)
import TypedSvg.Attributes exposing (class, fontFamily, fontSize, points, strokeWidth, textAnchor, transform, viewBox)
...
pointCircle scaleX scaleY pointClass xyPoint =
...
pointShape : ContinuousScale Float -> ContinuousScale Float -> String -> Point -> Svg msg
pointShape scaleX scaleY pointClass xyPoint =
    let
        px =
...
pointSize : ContinuousScale Float -> ContinuousScale Float -> String -> Point -> Svg msg
pointSize scaleX scaleY pointClass xyPoint =
    let
        localRadius =
            case pointClass of
                "other" ->
    ...
    g [ class [ "point", pointClass ], fontSize <| Px 10.0, fontFamily [ "sans-serif" ] ]
        ...
        , text_
            [ x <| Scale.convert scaleX xyPoint.x
            , y <| Scale.convert scaleY xyPoint.y - localRadius
            , textAnchor AnchorMiddle
            ]
            [ TypedSvg.Core.text xyPoint.pointName ]
        ]
...
pointsFromCars : List Car -> List Point
...
plotValues : PartitionedCars -> { otherPoints : List Point, belowPoints : List Point, abovePoints : List Point, xValues : List Float, yValues : List Float, xScaleLocal : ContinuousScale Float, yScaleLocal : ContinuousScale Float, labelX : Float, labelY : Float }
plotValues model =
    let
        otherPoints : List Point
        otherPoints =
            pointsFromCars model.otherCars

        belowPoints : List Point
        belowPoints =
            pointsFromCars model.belowAverage

        abovePoints : List Point
        abovePoints =
            pointsFromCars model.aboveAverage

        allPoints : List Point
        allPoints =
            otherPoints ++ belowPoints ++ abovePoints

        xValues : List Float
        xValues =
            if List.isEmpty allPoints then
                [ 0 ]

            else
                List.map .x allPoints
    ...
    , xValues = xValues
    , yValues = yValues
    , xScaleLocal = xScaleLocal
    , yScaleLocal = yScaleLocal
    , labelX = wideExtent xValues |> (\t -> (Tuple.second t - Tuple.first t) / 2)
    , labelY = wideExtent yValues |> Tuple.second
    }


renderPlot : String -> String -> (ContinuousScale Float -> ContinuousScale Float -> String -> Point -> Svg msg) -> PartitionedCars -> Svg msg
renderPlot plotClass extraCss drawPoint model =
    let
        p =
            plotValues model
    in
    svg [ class [ plotClass ], viewBox 0 0 w h, TypedSvg.Attributes.width <| TypedSvg.Types.Percent 100, TypedSvg.Attributes.height <| TypedSvg.Types.Percent 100 ]
        ...
        , g [ transform [ Translate padding (h - padding) ] ]
            [ xAxis p.xValues
            , text_
                [ x (Scale.convert p.xScaleLocal p.labelX)
                , y 30
                , textAnchor AnchorMiddle
                ]
        ...
        , g [ transform [ Translate padding padding ] ]
            ...
            , text_
                [ x 0
                , y (Scale.convert p.yScaleLocal p.labelY - 20)
                , textAnchor AnchorMiddle
                ]
                [ TypedSvg.Core.text "retailPrice" ]
            ]
        , g [ transform [ Translate padding padding ] ]
            (List.concat
                ...
                , List.map (drawPoint p.xScaleLocal p.yScaleLocal "above") p.abovePoints
                ]
            )
        ]
...
scatterplotV1 model =
    renderPlot
        "plot-v1"
        ".plot-v1 .point.above circle {stroke: rgba(0, 0, 0,0.4); fill: rgb(46, 139, 87); }\n .plot-v1 .point.other circle, .plot-v1 .point.other rect, .plot-v1 .point.other polygon {stroke: rgba(0, 0, 0,0.4); fill: rgb(240, 240, 240); }\n .plot-v1 .point.below circle, .plot-v1 .point.below rect, .plot-v1 .point.below polygon {stroke: rgba(0, 0, 0,0.4); fill: rgb(200, 200, 200); }"
        pointCircle
        model


scatterplotV2 : PartitionedCars -> Svg msg
scatterplotV2 model =
    renderPlot
        "plot-v2"
        ".plot-v2 .point.above polygon {stroke: rgba(0, 0, 0,0.4); fill: rgb(46, 139, 87); }\n .plot-v2 .point.other circle, .plot-v2 .point.other rect, .plot-v2 .point.other polygon {stroke: rgba(0, 0, 0,0.4); fill: rgb(240, 240, 240); }\n .plot-v2 .point.below circle, .plot-v2 .point.below rect, .plot-v2 .point.below polygon {stroke: rgba(0, 0, 0,0.4); fill: rgb(200, 200, 200); }"
        pointShape
        model


scatterplotV3 : PartitionedCars -> Svg msg
scatterplotV3 model =
    renderPlot
        "plot-v3"
        ".plot-v3 .point.above circle {stroke: rgba(0, 0, 0,0.4); fill: rgb(239, 239, 239); }\n .plot-v3 .point.other circle, .plot-v3 .point.other rect, .plot-v3 .point.other polygon {stroke: rgba(0, 0, 0,0.4); fill: rgb(240, 240, 240); }\n .plot-v3 .point.below circle, .plot-v3 .point.below rect, .plot-v3 .point.below polygon {stroke: rgba(0, 0, 0,0.4); fill: rgb(200, 200, 200); }"
        pointSize
        model
...
main =
    let
        ...
        averageVerbrauch : Float
        averageVerbrauch =
            filteredCars.averageCityMPG
                |> Maybe.withDefault 0.0

        countOther : Int
        countOther =
            List.length filteredCars.otherCars

        countBelow : Int
        countBelow =
            List.length filteredCars.belowAverage

        countAbove : Int
        countAbove =
            List.length filteredCars.aboveAverage
    in
    Html.div []
        [ Html.p []
            [ Html.text ("Gewählte Klasse: " ++ cclass) ]
        , Html.p []
            [ Html.text ("Durchschnittlicher Verbrauch (cityMPG): " ++ String.fromFloat averageVerbrauch) ]
        , Html.p []
            [ Html.text ("Autos nicht in Fahrzeugklasse: " ++ String.fromInt countOther) ]
        , Html.p []
            [ Html.text ("Autos unter durchschnittlichem Verbrauch: " ++ String.fromInt countBelow) ]
        , Html.p []
            [ Html.text ("Autos über durchschnittlichem Verbrauch: " ++ String.fromInt countAbove) ]
        , Html.h3 [] [ Html.text "Version 1: Farbe" ]
