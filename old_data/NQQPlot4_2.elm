module NQQPlot4_2 exposing (main)
...
pointCircle : ContinuousScale Float -> ContinuousScale Float -> String -> Point -> Svg msg
pointCircle scaleX scaleY pointClass p =
    let
        px =
            Scale.convert scaleX p.x

        py =
            Scale.convert scaleY p.y

        label =
            "(" ++ String.fromFloat p.x ++ ", " ++ String.fromFloat p.y ++ ")"
    in
    g [ class [ "point", pointClass ], fontSize <| Px 10.0, fontFamily [ "sans-serif" ] ]
        [ circle [ cx px, cy py, r radius ] []
        , text_ [ x px, y (py - radius), textAnchor AnchorMiddle ] [ TypedSvg.Core.text label ]
        ]


quantilePoints : List Float -> List Point
quantilePoints values =
    let
        sorted =
            List.sort values
        ...
        sorted
            |> List.indexedMap
                (\i v ->
                    let
                        f =
                            toFloat (i + 1) / (toFloat n + 1)

                        q =
                            Statistics.quantile f sorted |> Maybe.withDefault v
                    in
                    { pointName = String.fromInt (i + 1), x = f, y = q }
                )


qqPlotFromValues : List Float -> Svg msg
qqPlotFromValues values =
    let
        points =
            quantilePoints values
        ...
        xScaleLocal =
            Scale.linear ( 0, w - 2 * padding ) ( 0, 1 )

        yScaleLocal =
            yScale yVals
    in
    svg [ class [ "qq-plot" ], viewBox 0 0 w h, TypedSvg.Attributes.width <| TypedSvg.Types.Percent 100, TypedSvg.Attributes.height <| TypedSvg.Types.Percent 100 ]
        [ style []
            [ TypedSvg.Core.text
                (".qq-plot .point circle { fill: rgb(200, 200, 200); stroke: rgba(0,0,0,0.2); }\n"
                    ++ ".qq-plot .point text { display: none; font-family: sans-serif; font-size: 10px; }\n"
                    ++ ".qq-plot .point:hover circle { stroke: rgba(0, 0, 0,1.0); fill: rgb(118, 214, 78); }\n"
                    ++ ".qq-plot .point:hover text { display: inline; }"
                )
            ]
        , g [ transform [ Translate padding (h - padding) ] ]
            [ xAxis xVals
            , text_
                [ x (Scale.convert xScaleLocal 0.5)
                , y 30
                , textAnchor AnchorMiddle
                ]
                [ TypedSvg.Core.text "F-Value" ]
            ]
        , g [ transform [ Translate padding padding ] ]
            [ yAxis yVals
            , text_
                [ x 0
                , y (Scale.convert yScaleLocal (wideExtent yVals |> Tuple.second) - 20)
                , textAnchor AnchorMiddle
                ]
                [ TypedSvg.Core.text "cityMPG" ]
            ]
        , g [ transform [ Translate padding padding ] ]
            (List.map (pointCircle xScaleLocal yScaleLocal "") points)
        ]


qqPlotForPartition : PartitionedCars -> Svg msg
qqPlotForPartition model =
    let
        vals =
            List.map .cityMPG model.chosenComplete
    in
    qqPlotFromValues vals


qqPointsBetweenClasses : List Float -> List Float -> List Point
qqPointsBetweenClasses valuesA valuesB =
    let
        fs =
            quantilePoints valuesA |> List.map .x
    ...
    fs
        |> List.filterMap
            (\f ->
                case ( Statistics.quantile f sortedA, Statistics.quantile f sortedB ) of
                    ( Just qa, Just qb ) ->
                        Just { pointName = "", x = qa, y = qb }
...
qqPlotBetweenClasses : String -> String -> List Float -> List Float -> Svg msg
qqPlotBetweenClasses classA classB valuesA valuesB =
    let
        points =
            qqPointsBetweenClasses valuesA valuesB
        ...
        scaleXLocal =
            Scale.linear ( 0, w - 2 * padding ) domain

        scaleYLocal =
            Scale.linear ( h - 2 * padding, 0 ) domain
        ...
        [ style []
            [ TypedSvg.Core.text
                (".qq-plot .point circle { fill: rgb(200, 200, 200); stroke: rgba(0,0,0,0.2); }\n"
                    ...
                    ++ ".qq-plot .point:hover circle { stroke: rgba(0, 0, 0,1.0); fill: rgb(118, 214, 78); }\n"
                    ++ ".qq-plot .point:hover text { display: inline; }\n"
                    ++ ".qq-plot .diag { stroke: rgba(200, 80, 80, 0.9); stroke-width: 2; stroke-dasharray: 5,4; }"
                )
            ]
        , g [ transform [ Translate padding (h - padding) ] ]
            [ xAxis allVals
            , text_
                [ x (Scale.convert scaleXLocal ((dMin + dMax) / 2))
                , y 30
                , textAnchor AnchorMiddle
                ]
                [ TypedSvg.Core.text ("Quantile " ++ classA) ]
            ]
        , g [ transform [ Translate padding padding ] ]
            [ yAxis allVals
            , text_
                [ x 0
                , y (Scale.convert scaleYLocal dMax - 20)
                , textAnchor AnchorMiddle
                ]
                [ TypedSvg.Core.text ("Quantile " ++ classB) ]
            ]
        , g [ transform [ Translate padding padding ] ]
            [ line
                [ class [ "diag" ]
                , x1 (Scale.convert scaleXLocal dMin)
                , y1 (Scale.convert scaleYLocal dMin)
                , x2 (Scale.convert scaleXLocal dMax)
                , y2 (Scale.convert scaleYLocal dMax)
                ]
                []
            ]
        , g [ transform [ Translate padding padding ] ]
            (List.map (pointCircle scaleXLocal scaleYLocal "") points)
        ]
...
normalQQPoints : List Float -> List Point
normalQQPoints values =
    let
        sorted =
            List.sort values
        ...
        mu =
            List.sum sorted / toFloat n

        std =
            Statistics.deviation sorted |> Maybe.withDefault 0.0
    in
    if n == 0 then
        []
            ...
            |> List.indexedMap
                (\i v ->
                    let
                        ...
                        qn =
                            mu + std * invNormalCdf f
                    in
                    { pointName = String.fromInt (i + 1), x = qn, y = v }
                )


normalQQPlotFromValues : String -> List Float -> Svg msg
normalQQPlotFromValues className values =
    let
        points =
            normalQQPoints values
        ...
        mu =
            List.sum sorted / toFloat (max 1 n)
        ...
        scaleXLocal =
            Scale.linear ( 0, w - 2 * padding ) domainX

        scaleYLocal =
            Scale.linear ( h - 2 * padding, 0 ) domainY

        q_e f =
            Statistics.quantile f sorted |> Maybe.withDefault 0.0
        ...
        maxXBox =
            Tuple.second domainX

        minYBox =
            Tuple.first domainY

        maxYBox =
            Tuple.second domainY
    ...
    svg [ class [ "qq-plot" ], viewBox 0 0 w h, TypedSvg.Attributes.width <| TypedSvg.Types.Percent 100, TypedSvg.Attributes.height <| TypedSvg.Types.Percent 100 ]
        ...
        , g [ transform [ Translate padding (h - padding) ] ]
            [ xAxis xVals
            , text_
                [ x (Scale.convert scaleXLocal ((minXBox + maxXBox) / 2))
                , y 30
                , textAnchor AnchorMiddle
                ]
                [ TypedSvg.Core.text "Theoretische Quantile (Normalverteilung)" ]
            ]
        , g [ transform [ Translate padding padding ] ]
            [ yAxis yVals
            , text_
                [ x 0
                , y (Scale.convert scaleYLocal (Tuple.second domainY) - 20)
                , textAnchor AnchorMiddle
                ]
                [ TypedSvg.Core.text ("Empirische Quantile (" ++ className ++ ")") ]
            ]
        , g [ transform [ Translate padding padding ] ]
            [ line
                [ class [ "diag" ]
                , x1 (Scale.convert scaleXLocal startX)
                , y1 (Scale.convert scaleYLocal startY)
                , x2 (Scale.convert scaleXLocal endX)
                , y2 (Scale.convert scaleYLocal endY)
                ]
                []
            ]
        , g [ transform [ Translate padding padding ] ]
...
type alias PartitionedCars =
    ...
    , averageCityMPG : Maybe Float
    , chosenComplete : List CompleteEntry
    }
...
completeEntries : List Car -> List CompleteEntry
completeEntries myCars =
    myCars
        |> List.filterMap
            (\car ->
                Maybe.map4
                    (\city retail dealer len ->
                        { car = car
                        , cityMPG = toFloat city
                        , retailPrice = toFloat retail
                        , dealerCost = toFloat dealer
                        , carLen = toFloat len
                        }
                    )
                    car.cityMPG
...
cityValuesForCarType : CarType -> List Car -> List Float
cityValuesForCarType carType myCars =
    completeEntries myCars
        |> List.filter (\entry -> entry.car.carType == carType)
        |> List.map .cityMPG
...
filterAndReduceCars myCars =
    let
        ...
        chosenCars =
            completeCars
                |> List.filter (\entry -> entry.car.carType == chosenCarType)
            ...
            case averageCityMPG of
                ...
                Just avg ->
                    let
                        below =
                            chosenCars
                                |> List.filter (\entry -> entry.cityMPG < avg)
                                |> List.map .car
    ...
    , averageCityMPG = averageCityMPG
    , chosenComplete = chosenCars
    }
...
main =
    let
        ...
        class1Name : String
        class1Name =
            carTypeToString targetClass

        rawValues =
            completeEntries cars
                |> List.filter (\entry -> entry.car.carType == targetClass)
                |> List.map .retailPrice

        logVals =
            List.map (\x -> logBase 10 x) rawValues

        powVals1 =
            List.map (\x -> x ^ 0.5) rawValues

        powVals2 =
            List.map (\x -> x ^ -1.0) rawValues
    in
    Html.div []
        [ Html.p []
            [ Html.text ("Gewählte Autoklasse: " ++ class1Name ++ " | Attribut: retailPrice | Anzahl: " ++ String.fromInt (List.length rawValues)) ]
        , Html.h3 [] [ Html.text "Originaldaten (retailPrice)" ]
        , normalQQPlotFromValues class1Name rawValues
        , Html.h3 [] [ Html.text "Log-Transformation (log10(retailPrice))" ]
        , normalQQPlotFromValues class1Name logVals
        , Html.h3 [] [ Html.text "Power-Transformation (Wurzel: retailPrice^0.5)" ]
        , normalQQPlotFromValues class1Name powVals1
        , Html.h3 [] [ Html.text "Power-Transformation (Kehrwert: retailPrice^-1)" ]
        , normalQQPlotFromValues class1Name powVals2
        ]
