module Scatterplot2_23 exposing (main)
...
import TypedSvg.Attributes exposing (class, fontFamily, fontSize, textAnchor, transform, viewBox, fill, fillOpacity, stroke)
...
pointV1 : ContinuousScale Float -> ContinuousScale Float -> PointEx -> Svg msg
pointV1 scaleX scaleY xyPoint =
    let
        ptColor = case xyPoint.subset of
            Above -> Color.rgb255 76 175 80
            Other -> Color.rgb255 200 200 200
    ...
    g [ class [ "point" ], fontSize <| Px 10.0, fontFamily [ "sans-serif" ] ]
        [ circle
            ...
            , stroke <| Paint (Color.rgba 0 0 0 0.5)
            ]
            []
        , text_ [ x <| Scale.convert scaleX xyPoint.x, y <| 0 - radiusSize, textAnchor AnchorMiddle ] [ TypedSvg.Core.text xyPoint.pointName ]
        ]

pointV2 : ContinuousScale Float -> ContinuousScale Float -> PointEx -> Svg msg
pointV2 scaleX scaleY xyPoint =
    let
        ptColor = case xyPoint.subset of
            Above -> Color.rgb255 76 175 80
            Other -> Color.rgb255 200 200 200
        ...
        radiusSize = 5.0
        xPos = Scale.convert scaleX xyPoint.x
        yPos = Scale.convert scaleY xyPoint.y
    in
    g [ class [ "point" ], fontSize <| Px 10.0, fontFamily [ "sans-serif" ] ]
        [ case xyPoint.subset of
            Above ->
                rect
                    [ x (xPos - radiusSize)
                    , y (yPos - radiusSize)
            ...
            Other ->
                circle
                    ...
                    []
        , text_ [ x xPos, y <| 0 - radiusSize, textAnchor AnchorMiddle ] [ TypedSvg.Core.text xyPoint.pointName ]
        ]

pointV3 : ContinuousScale Float -> ContinuousScale Float -> PointEx -> Svg msg
pointV3 scaleX scaleY xyPoint =
    let
        ptColor = case xyPoint.subset of
            Above -> Color.rgb255 76 175 80
            Other -> Color.rgb255 200 200 200
            
        radiusSize = case xyPoint.subset of
            Above -> 9.0
            Other -> 4.0
    in
    g [ class [ "point" ], fontSize <| Px 10.0, fontFamily [ "sans-serif" ] ]
...
scatterplot : (ContinuousScale Float -> ContinuousScale Float -> PointEx -> Svg msg) -> XyDataEx -> Svg msg
scatterplot pointRenderer model =
    let
    ...
    svg [ viewBox 0 0 w h, TypedSvg.Attributes.width <| TypedSvg.Types.Percent 100, TypedSvg.Attributes.height <| TypedSvg.Types.Percent 100 ]
        ...
        , g [ transform [ Translate padding padding ] ]
            (List.map (pointRenderer xScaleLocal yScaleLocal) model.data)
        ]


type Subset = Above | Other

type alias PointEx =
    { pointName : String, x : Float, y : Float, subset : Subset }
...
main =
    ...
    Html.div []
        [ Html.p []
            [ Html.text ("Gewaehlte Klasse: " ++ cclass) ]
        , Html.p []
        ...
        , Html.p []
            [ Html.text ("Autos ueber durchschnittlichem Verbrauch: " ++ String.fromInt countAbove) ]
