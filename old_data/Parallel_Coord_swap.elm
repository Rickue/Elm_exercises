module Parallel_Coord_swap exposing (main)
...
import Statistics
import TypedSvg exposing (g, path, style, svg, text_)
import TypedSvg.Attributes exposing (class, d, fill, textAnchor, transform, viewBox)
...
view model =
    let
        filteredCars =
            completeEntries model.dimensions cars

        modelForPlot =
            { dimDescription = List.map Tuple.first model.dimensions
            , data = [ filteredCars ]
            }
    in
    div []
        [ p []
            [ text "Dimensionen verschieben mit den Buttons." ]
        , div []
            (List.indexedMap
                (\i ( name, _ ) ->
                    div []
                        [ text name
                        , button [ onClick (MoveLeft i) ] [ text "<" ]
                        , button [ onClick (MoveRight i) ] [ text ">" ]
                        ]
                )
                model.dimensions
            )
        , parallelCoodinatesPlot w h modelForPlot
        ]
    ...
    case msg of
        MoveLeft i ->
            if i <= 0 then
                ...
                { model | dimensions = swapAt i (i - 1) model.dimensions }
...
wideExtent values =
    let
        ( min, maxim ) =
            Statistics.extent values |> Maybe.withDefault ( 0, 100 )
    in
    if min == maxim then
        ( min - 1, maxim + 1 )
...
parallelCoodinatesPlot width height model =
    let
        nDimensions =
        ...
        valuesForDim : Int -> List Float
        valuesForDim i =
            List.filterMap (\p -> List.drop i p.value |> List.head) allPoints

        yScales : List (ContinuousScale Float)
        yScales =
            List.range 0 (nDimensions - 1)
                |> List.map (\i -> Scale.linear ( height, 0 ) (wideExtent (valuesForDim i)))

        xScale : ContinuousScale Float
        xScale =
            Scale.linear ( 0, width ) ( 1, toFloat nDimensions )
        ...
        drawAxes =
            List.indexedMap
                (\i dimName ->
                    let
                        scale =
                            List.drop i yScales |> List.head |> Maybe.withDefault (Scale.linear ( height, 0 ) ( 0, 100 ))

                        xPos =
                            Scale.convert xScale (toFloat (i + 1))
                    in
                    g [ transform [ Translate xPos 0 ] ]
                        [ Axis.left [ Axis.tickCount 5 ] scale
                        , text_ [ x 0, y -10, textAnchor AnchorMiddle ] [ TypedSvg.Core.text dimName ]
                        ]
                )
                                    ...
                                    case coords of
                                        ...
                                        ( sx, sy ) :: rest ->
                                            "M"
                                                ...
                                                ++ String.fromFloat sy
                                                ++ " "
                                                ++ String.join " " (List.map (\( cx, cy ) -> "L" ++ String.fromFloat cx ++ "," ++ String.fromFloat cy) rest)
                            in
                            path [ d pathStr, fill PaintNone, class [ "pc-line" ] ] []
                        )
                        pointList
                )
                model.data
    in
    svg [ viewBox 0 0 (width + 2 * padding) (height + 2 * padding), TypedSvg.Attributes.width <| TypedSvg.Types.Percent 100, TypedSvg.Attributes.height <| TypedSvg.Types.Percent 100 ]
        [ style []
            [ TypedSvg.Core.text
                (".pc-line { stroke: rgb(10, 10, 10); stroke-width: 1.5; fill: none; }\n"
                    ++ ".pc-line:hover { stroke: rgb(118, 214, 78); stroke-width: 3; }"
                )
            ]
...
completeEntries : List Dimension -> List Car -> List MultiDimPoint
completeEntries dimensions myCars =
    myCars
        |> List.filter (\c -> c.carType == chosenCarType)
        |> List.filterMap
            (\car ->
                Maybe.map4
                    (\city retail dealer len ->
                        ...
                        in
                        { pointName = filteredCar.vehicleName
                        , value =
                            List.map
                                (\accessor -> toFloat (accessor filteredCar))
                                (List.map Tuple.second dimensions)
                        }
                    )
                    car.cityMPG
                    car.retailPrice
